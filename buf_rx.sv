...
// 每个队列保存最近一次完成块的地址与元信息，避免补链时依赖同步RAM读出的旧 tail 值
reg  [11:0]   tail_addr[0:31];
reg  [14:0]   tail_meta[0:31]; // {pkt_ed, slice_cnt[2:0], pkt_len[10:0]}
integer       k;
...
always@(posedge clk or negedge rst_n) begin
    if(~rst_n) begin
        ...
        if (update_prev) begin
            ...
            buf_list_info_waddr  <= tail_addr[qid];
            ...
            // 后一拍：写当前块的完整链表条目，同时把当前块登记成新的 tail
            buf_list_info_wen    <= 1'b1;
